...
roc.plot(test_set$depressed, pred)
```
...
```{r}
control <- trainControl(method = "cv", p = .9)
train_knn <- train(train_set[, -which(names(train_set) == "depressed")], 
                   train_set$depressed, method = "knn", 
                   tuneGrid = data.frame(k = seq(3,20, 2)), trControl = control)
train_knn
fit_knn <- knn3(train_set[, -which(names(train_set) == "depressed")], 
                factor(train_set$depressed),  k = 3)
