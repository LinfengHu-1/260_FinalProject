...
Therefore we can conclude that this general linear model is not sufficient to depict the relationship of the covariates listed and the outcome of interest depression status. We proceed with other machine learning approaches, mostly ensembled methods that can hopefully better characterize the relationships between depression onset with socioeconomic and health factors. Algorithms with all three types(bagging, boosting and stacking) of ensembled learning techniques were used to compare for the best-performing one that is most suitable for this dataset. 
