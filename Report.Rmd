---
title: "BST260 Final Report: Depression prediction with individual-level factors"
author: Linfeng Hu
...
I then chose to examine the covariates that I have a clear understanding for, since we need to understand potential correlations in the model. There are only 20 features left for modeling the association. I used an influence plot which graphs the studentized residuals against hat values(fitted values made by my logistic regression model), and the sizes and fill color of the red circles are proportional to Cook’s distance. Bigger the size of the circle, higher the influence this data point has to the regression analysis. We can see that there are around 4 points with large studentized residuals with their Cook’s distances labeled. Although these 4 points are labeled due to the magnitude of their residual values, none of the points shows an absolute value larger than 3. Therefore, we can temporarily conclude that there is no obvious outlier in our data set.

  ![ ](./influencePlot.png){width=50%}

From the correlation matrix of all our covariates, we can observe that factors that are highly correlated (correlation > 0.6) include hhsize & children & hh_totalmembers & hhchildren, cons_nondurable and cons_allfood and cons_med_total. Therefore, we proceed to eliminate covariates that are highly correlated in our model. After further cleaning of colinear factors, we can obtain the VIP scores for most influential factors below. 

  ![ ](./vipScore.png){width=60%}

A VIP score is the measure of a variable's importance for the overall model. From the plot, we can observe that the factors that seem to be contributing the most to the outcome depression are the size of household, proportion of household sick or injured in the last month, marital status and years of education completed. These covariates mentioned seem to point to common sources of stress and emotional turbulence that may be introduced in one's life. From this data set, these factors seem to have high importance in explaining the variance of the dependent variable, and is therefore closely associated with the depression outcome.

### Description  & justification of Methodology

The data set is heavily imbalanced with only 16% of participants having the outcome of interest. Using imbalanced data set may result in poor outcomes in the ROC-AUC(receiver operating characteristic and area under the curve scores) result. Therefore we moved forward with changing the cutoff threshold for prediction from the general default 0.5.

From the ROC curve of a general logistic regression fitted for this data set in the exploratory data analysis, we can see that it comes quite closer to the diagonal line, indicating low accuracy. From the general linear model summary, it is also observed that most of the covariates are not statistically significant in relation to outcome variable. This may be due to the fact that the linearity assumption in the predictors does not hold well, which in turn affects the performance of a untuned logistic regression model. 

Therefore we can conclude that this general linear model is not sufficient to depict the relationship of the covariates listed and the outcome of interest depression status. We proceed with other machine learning approaches, such as ensembled methods that can hopefully better characterize the relationships between depression onset with socioeconomic and health factors than one single classifier. Algorithms with all three types(bagging, boosting and stacking) of ensembled learning techniques were used to compare for the best-performing one that is most suitable for this dataset. 

## Results

I first dropped unnecessary features for our analysis such as village identifier, personal identification number and the survey date. Then, I create initial data partition for further examination using training set (containing 80% of observations) and test sets (containing 20% of observations). 
